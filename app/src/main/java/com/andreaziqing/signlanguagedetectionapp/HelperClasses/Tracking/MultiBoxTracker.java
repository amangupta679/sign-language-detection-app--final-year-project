package com.andreaziqing.signlanguagedetectionapp.HelperClasses.Tracking;
...
public class MultiBoxTracker {
    ...
    private final String MULTI_BOX_TRACKER = "Multi box Tracker";
    ...
    public synchronized void trackResults(final List<Detector.Recognition> results, final long timestamp) {
        Log.i(MULTI_BOX_TRACKER, "Processing " + results.size() + " results from " + timestamp);
    ...
    private void processResults(final List<Detector.Recognition> results) {
        ...
        for (final Detector.Recognition result : results) {
            ...
            Log.v(MULTI_BOX_TRACKER,
                    "Result! Frame: " + result.getLocation() + " mapped to screen:" + detectionScreenRect);

            screenRects.add(new Pair<Float, RectF>(result.getConfidence(), detectionScreenRect));

            if (detectionFrameRect.width() < MIN_SIZE || detectionFrameRect.height() < MIN_SIZE) {
                Log.w(MULTI_BOX_TRACKER, "Degenerate rectangle! " + detectionFrameRect);
                continue;
            }
        ...
        if (rectsToTrack.isEmpty()) {
            Log.v(MULTI_BOX_TRACKER, "Nothing to track, aborting.");
            return;
        }
